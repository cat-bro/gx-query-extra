...
local_query-jobs-tool-has-stderr() { ## input tool substr,  stderr substr, optional limit
    tool_substr="$1"
	tool_stderr_substr="$2"
    limit="$3"
	[ ! "$3" ] && limit="50" || limit="$3"
	...
	read -r -d '' QUERY <<-EOF
			SELECT
				...
				(
					position('$tool_stderr_substr' in j.tool_stderr)>0
				) as has_stderr,
				j.destination_id as destination
			FROM job j
...
local_query-jobs-tool-has-stderr-command() { ## input tool substr,  stderr substr, optional limit
    tool_substr="$1"
	tool_stderr_substr="$2"
    limit="$3"
	[ ! "$3" ] && limit="50" || limit="$3"
