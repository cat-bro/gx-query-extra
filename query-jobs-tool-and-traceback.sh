local_query-jobs-tool-and-traceback() { ## input tool substr,  stderr substr, optional limit
    tool_substr="$1"
	tool_traceback_substr="$2"
    limit="$3"
	[ ! "$3" ] && limit="50" || limit="$3"
	handle_help "$@" <<-EOF
	EOF
	...
	read -r -d '' QUERY <<-EOF
			...
			FROM job j
			WHERE position('$tool_traceback_substr' in j.traceback)>0
