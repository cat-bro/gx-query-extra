local_query-nonterminal-jobs-by-object-store-id() { ## [object_store_ids]: Comma separated substrings of object store backend IDs, optional limit as second arg
	object_store_ids="$1"
	...
	read -r -d '' QUERY <<-EOF
			...
			AND j.state in ('new', 'queued', 'running', 'paused')
